...
public class OpenApiFetchService {
    ...
    public FscIndexResponseDto fetchStockMarketIndex(String targetDate, int numOfRows, int pageNo) {
        ...
        URI uri = UriComponentsBuilder.fromHttpUrl(openApiProperties.getBaseUrl() + "/getStockMarketIndex")
        ...
        try {
            ...
            log.error("[OpenAPI 통신 에러] 상태 코드: {}, 응답 본문: {}", e.getStatusCode(), e.getResponseBodyAsString());
            throw new BusinessException(ErrorCode.OPEN_API_COMMUNICATION_ERROR, "서버 응답 에러: " + e.getStatusCode());
        } catch (Exception e) {
            log.error("[OpenAPI 알 수 없는 에러] 요청 처리 중 장애 발생", e);
            throw new BusinessException(ErrorCode.OPEN_API_COMMUNICATION_ERROR, "네트워크/파싱 예외: " + e.getMessage());
        }
    }
}