...
public class IndexDataServiceImpl implements IndexDataService {
  ...
  public PagingResponse<IndexDataDto> getAll(IndexDataSearchCondition condition) {
    ...
    Long resolvedIdAfter =
        pagingMapper.resolveIdAfter(condition.getCursor(), condition.getIdAfter());
    condition.setIdAfter(resolvedIdAfter);

    List<IndexDataDto> results =
        indexDataRepository.search(condition).stream().map(this::toDto).toList();

    long totalElements = indexDataRepository.count(condition);

    return pagingMapper.toResponse(results, size, totalElements, IndexDataDto::id);
  }
